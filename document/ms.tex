...
box.
First, we filter the \pdc\ light curves \citep{Stumpe:2012, Smith:2012}
using a running windowed median with a half-width of 2~days to remove stellar
variability.
...
transit with no more than 500 cadences and compute the BIC of each model for
this data set.
...
We run each chain for at least 750 autocorrelation times\footnote{Estimated
using a robust iterative method as suggested by Alan Sokal:
...
orbital parameters and GP hyperparameters to find the maximum \foreign{a
posteriori} model then keep these.


\begin{floattable}
\begin{deluxetable}{lccc}
\tabletypesize{\footnotesize}
\tablecaption{The inferred parameters and priors used in the inference
...
\subsection{Catalog of candidates}

\begin{floattable}
\begin{deluxetable}{cccccl}
\tabletypesize{\footnotesize}
\tablecaption{The inferred parameters for the long-period transiting exoplanet
candidates
\label{tab:table}}
\include{paramtab}
...
\end{center}
\caption{%
The completeness of the search procedure described here as a function of
period and radius.
In each bin, the completeness (in percent) is given with the associated
