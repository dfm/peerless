...
\end{eqnarray}
\dfmtodo{Talk about doing integrals with this.}
...
\begin{eqnarray}
\frac{\dd N}{\dd\ln P\dd\ln R} &=&
    \frac{N(P_\mathrm{min},\,P_\mathrm{max};\,R_\mathrm{min},\,R_\mathrm{max})}
         {Z(P_\mathrm{min},\,P_\mathrm{max};\,R_\mathrm{min},\,R_\mathrm{max})}
\end{eqnarray}
where $N(\cdots)$ is the number of planets in the volume and
\begin{eqnarray}
Z(P_\mathrm{min},\,P_\mathrm{max};\,R_\mathrm{min},\,R_\mathrm{max}) &=&
    \sum_{k=1}^{K} \int p(w_{P,\,R})\,Q_k(w)\,\bvec{1}[P,\,R\in V]\dd w
\end{eqnarray}
where $p(w_{P,\,R})$ is the prior distribution of all the parameters except
the period and radius.
...
The occurrence rate units are ``expected number of planets per ln-radius,
per ln-period, per G/K dwarf''.}
\end{deluxetable}
\end{floattable}
