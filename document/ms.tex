...
the \kepler\ lifetime.
This neglects the small but non-negligible probability~--~less than one
percent for the typical candidate~--~that a second transit might have
...
actual normalizations are given in the text.
The green points show the shapes parameters of the long-period exoplanet
...
\begin{eqnarray}
\frac{\dd^2 N}{\dd\ln M/M_\mathrm{J}\,\dd\ln P/\unit{yr}}
    &\lesssim& 0.026
\end{eqnarray}
per M-dwarf.
...
Recently, \citep{Bryan:2016} studied the frequency of long-period giant
...
(\eqalt{massocc}) once again suggesting that cold-Jupiters preferentially
occur in systems with inner planets.
...
\end{eqnarray}
This value is lower than the value computed using our sample in
Table~\ref{tab:occrates} but this is consistent with the fact that direct
imaging is sensitive to larger planets at wider separations than the transit
method.
...
Simulations of the false positive population~--~the secondary eclipses of
binary systems and background eclipsing binaries~--~suggest that 13 of these
