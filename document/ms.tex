...
event in about 10~percent of targets and the three highest signal-to-noise
...
{\item a single outlier to account for a bad data point,}
{\item a step function to describe sudden pixel sensitivity dropouts (SPSDs;
\dfmtodo{CITE}), and}
{\item a box to catch signals that are well fit by the search scalar but
insufficiently transit-like to be convincing.}
\end{itemize}
The functional forms of these models are given in Appendix \dfmtodo{SOME APP}.

This method of automated vetting is clearly limited by the specific models
