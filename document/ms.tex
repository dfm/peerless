...
other model classes because, for example, the \modelname{box} model is
probably sensitive to other astrophysical phenomena like secondary eclipses.
In a cursory investigation it is clear that the majority of signals labeled
\modelname{box} in our analysis are noise but a subset are likely to be
...
Given the fact that only \numkois\ of the \numtargets\ targets have known
planets, this means that of the systems with known inner planets, \fracinner\
have outer transiting planets with one or two transits detectable by our
search.
This fraction is nearly a factor of 20 larger than the same number for targets
with no known inner transiting planets: $\fracnoinner$.
...
This suggests that these long-period planets are preferentially aligned with
...
We do not consider these events further in the rest of this \paper\ but
Table~\ref{tab:rejects} lists these events and their properties for
posterity.
...
package
\exosyspop\footnote{\url{https://github.com/timothydmorton/exosyspop}}, which
...
simulations of \citep{Morton:2011} at many different Galactic latitudes). Each
...
These results suggest that we might certainly expect to see some unblended
eclipsing binary signals in our search.  However, this does not mean that  we
should fear that $\sim$7 of the planet candidates might be EBs. Rather, we can
instead inspect the synthetic observation simulations in more detail; in
...
according to the two-bin population model described by Table~\ref{tab:occurrence} and
...
discussed in \sect{catalog}, 4754460 (which we calculate to have a 5\% false positive
...
will be followed up and precisely measured by the \gaia\ Mission.
