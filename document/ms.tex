...
See the last page for endnotes
...
Data from the \kepler\ Mission \citep{Borucki:2011} has been used to discover
...
interesting in their own right but the development of a method for the
discovery of planets with orbital periods longer than the survey baseline are
crucial for the future of the study of exoplanets via the transit method.
...
\plato, \citealt{Rauer:2014}).
The methodology presented in the following pages can be modified to be applied
to these forthcoming data sets as they are collecting in the upcoming years.
...
sensitive to smaller planets than the current state-of-the-art radial velocity
surveys.
...
Mission, release a catalog of exoplanet candidates, and estimate the
occurrence rate of long-period exoplanets.
We finish by discussing the potential effects of false positives, the
prospects for radial velocity follow-up of these candidates, and compare to
calculated occurrence rate to results based on discoveries using different
...
\include{ortab}
\tablenotetext{a}{$\Gamma_V = \dd^2 N/\dd\ln P\dd\ln R$; see \eq{rate-density}}
\tablenotetext{b}{$\Gamma_V$ computed assuming that one candidate is a false
positive; see \eq{rate-minus}}
...
transiting planet \citep{}.  However, the occurrence rate calculation
...
package \exosyspop\ \citep{Morton:2016b}, which we developed for this
purpose.
...
objects potentially detectable via either primary or secondary eclipse
(according to randomly oriented orbital geometries).  This synthetic catalog
includes signal-to-noise estimates of both the primary and secondary eclipses,
the number of detected primary and secondary eclipses, and the trapezoidal
shape parameters (depth, duration, and ingress-to-duration ratio, as defined
in \citet{Morton:2012}) of each detection.
...
find that a binary fraction of 15\% between periods of 20 days and 25 years, with
periods distributed as $f(P) \sim P^{-1.1}$ and eccentrities distributed according
...
The scenarios typically considered in \kepler\ false positive analysis
\citep[][e.g.]{Morton:2016} are unblended eclipsing binary stars (EB),
background/foreground chance-aligned eclipsing binaries (BEB), and
hierarchical triple eclipsing binaries (HEB).  In this work, where
we are concerned with long-period signals, we do not consider the HEB
scenario, as most known hierarchical stellar systems have two components on
relatively short orbits \citep{}.

\tdmtodo{Just a note that I've added your figure in at the end of this section
--- DFM}
...
uncertainty introduced by false positives.
...
These methods all measure the occurrence rate in units of mass and semi-major
axis instead of radius and period.
Using the transit method, we do not directly measure the mass of the planet.
Instead, we must rely on a mass--radius (M--R) relationship constructed using
exoplanets with both mass and radius measurements \citep[for
example][]{Weiss:2014, Wolfgang:2016, Chen:2016} to predict the expected
masses of the transiting planets.
...
We compared the predictions with the predictions from \citet{Wolfgang:2016}
and found similar values with smaller uncertainties and chose to use the
\citet{Chen:2016} relationship because it was more conservative in the
relevant range of parameter space.
...
This effect would probably cause the masses of these cool planets to be
systematically underestimated.

Using the same M--R relationship, we can also compute the completeness of our
...
\frac{\dd^2 N}{\dd\ln M\,\dd\ln a} &=& \massocc
\end{eqnarray}
where $N$ is the expected number of planets per G/K dwarf.
The uncertainty in \eq{massocc} does not take into account the uncertainties
in the semi-major axis or mass estimates or any systematic noise in the
mass--radius relationship.
Therefore, the specific result should be taken with the appropriate grain of
salt but a result in this parameter space eases qualitative comparison with
occurrence rates measured using different methods.
...
\hoggtodo{Comments about GAIA follow-up}
...
to be on the order of 40\,$\mu$as at these magnitudes \hoggtodo{cite
something}.
...
However, it is not expected that \gaia\ can detect or measure precisely
...
It also means that many of the discoveries of the \tess\ Mission will be
followed up and precisely measured by the \gaia\ Mission.
