...
Instead, we must rely on a mass--radius relationship constructed using
...
mass--radius relationship is beyond the scope of this \paper\ but it is worth
...
We find that the mean occurrence rate density in the range $0.01\,M_\mathrm{J}
\le M < 20\,M_\mathrm{J}$ and $1.5\unit{au} \le a < 9\unit{au}$ is
...
\tabletypesize{\footnotesize}
\caption{The mass and semi-major axis estimates for the candidates from
Table~\ref{tab:catalog} \label{tab:masses}}
\include{masstab}
